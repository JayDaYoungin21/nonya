...
const IS_CLOUD =
  process.env.CLOUD === "1" ||
  process.env.CLOUD_RUN === "1" ||
  process.env.AWS_LAMBDA_FUNCTION_NAME ||
  process.env.K_SERVICE ||
  process.env.WEBSITE_INSTANCE_ID ||
  process.env.CI === "true";
const DEBUG_LOGS = process.env.DEBUG_LOGS === "1" || IS_CLOUD;
const DEFAULT_ACCEPT_LANGUAGE = "en-US,en;q=0.9";
const DEFAULT_DOWNLOAD_UA =
  "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36";
const LOG_LEVEL = (process.env.LOG_LEVEL || (IS_CLOUD ? "debug" : "info")).toLowerCase();
...
function logEvent(level, message, extra) {
  ...
  const payload = { level, ts: new Date().toISOString(), msg: message };
  if (extra !== undefined) payload.extra = extra;
  console.log(JSON.stringify(payload));
}

function logStep(message, extra) {
  logEvent("info", message, extra);
}

function logDebug(message, extra) {
  logEvent("debug", message, extra);
}

async function withEvalLog(label, fn) {
  const start = Date.now();
  logDebug(`[eval] start ${label}`);
  try {
    const result = await fn();
    logDebug(`[eval] end ${label}`, { elapsedMs: Date.now() - start });
    return result;
  } catch (err) {
    logEvent("warn", `[eval] fail ${label}`, {
...
async function waitForXPath(page, xpath, timeout = 60000) {
  logDebug("waitForXPath", { xpath, timeout });
...
function downloadToFile(url, filePath, headers = {}) {
  logDebug("downloadToFile", { url: formatUrlForLog(url), filePath });
...
function downloadToFileWithCurl(url, filePath, headers = {}) {
  logDebug("downloadToFileWithCurl", { url: formatUrlForLog(url), filePath });
  return new Promise((resolve, reject) => {
...
async function downloadWithCookies(page, url, filePath) {
  logStep("downloadWithCookies", { url: formatUrlForLog(url), filePath });
  let cookieHeader = "";
  let userAgent = "";
  let language = DEFAULT_ACCEPT_LANGUAGE;
  try {
    const cookies = await withEvalLog("page.cookies", () => page.cookies(url));
    cookieHeader = cookies.map((c) => `${c.name}=${c.value}`).join("; ");
  } catch (err) {
    logEvent("warn", "Cookie fetch failed, continuing without cookies.", err.message);
  }
  try {
...
async function downloadFromPage(page, url, filePath) {
  if (url.startsWith("blob:")) {
    const base64 = await withEvalLog("downloadFromPage:blob", () =>
      page.evaluate(async (blobUrl) => {
...
async function collectImageSources(page) {
  return withEvalLog("collectImageSources", () =>
    page.evaluate((xp) => {
      const results = [];
...
async function isImageReadyBySrc(page, src) {
  return withEvalLog("isImageReadyBySrc", () =>
    page.evaluate((targetSrc) => {
...
async function logImageDom(page, src, label) {
  const html = await withEvalLog("logImageDom", () =>
    page.evaluate((targetSrc) => {
  ...
  if (html) {
    logDebug(`[image-dom] ${label}`, html);
  } else {
    logDebug(`[image-dom] ${label}: element not found for src.`);
  }
}

async function clickXPath(page, xpath) {
  logDebug("clickXPath", { xpath });
...
async function realClick(page, xpath, delayMs = 250) {
  logDebug("realClick", { xpath, delayMs });
  const el = await waitForXPath(page, xpath);
  await el.evaluate((node) => {
    node.scrollIntoView({ block: "center", inline: "center" });
  });
...
async function typeXPath(page, xpath, value) {
  logDebug("typeXPath", { xpath, length: String(value || "").length });
  const el = await waitForXPath(page, xpath);
  await el.evaluate((node) => {
    node.scrollIntoView({ block: "center", inline: "center" });
  });
...
async function typeIntoForm(page, xpath, value) {
  logDebug("typeIntoForm", { xpath, length: String(value || "").length });
  const el = await waitForXPath(page, xpath);
  await el.evaluate((node) => {
    node.scrollIntoView({ block: "center", inline: "center" });
  });
...
async function getTextByXPath(page, xpath) {
  logDebug("getTextByXPath", { xpath });
  const el = await waitForXPath(page, xpath);
  const text = await withEvalLog("getTextByXPath:evaluate", () =>
...
async function getTextIfExistsByXPath(page, xpath) {
  const text = await withEvalLog("getTextIfExistsByXPath", () =>
    page.evaluate((xp) => {
      const node = document.evaluate(
      xp,
      document,
...
async function waitForXPathPresence(page, xpath, timeoutMs = 60000, pollMs = 500) {
  logDebug("waitForXPathPresence", { xpath, timeoutMs, pollMs });
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const exists = await withEvalLog("waitForXPathPresence:evaluate", () =>
      page.evaluate((xp) => {
        return Boolean(
          document.evaluate(
            xp,
...
async function getTurnstileTokenStatus(page) {
  return withEvalLog("getTurnstileTokenStatus", () =>
    page.evaluate(() => {
...
function startRateLimitWatcher(page, state, pollMs = 2000) {
  logDebug("startRateLimitWatcher", { pollMs });
  let stopped = false;
  const task = (async () => {
    while (!stopped) {
      if (!state.rateLimited) {
        try {
          const match = await withEvalLog("rateLimitWatcher:evaluate", () =>
            page.evaluate((xp, expected) => {
            const node = document.evaluate(
              xp,
              document,
              null,
          ...
          if (match) {
            ...
            logEvent("warn", "[rate-limit] Upgrade to unlock more detected. Restarting session.");
          }
        } catch (err) {
...
async function prepareImagine(page, state, modality) {
  ...
  logStep("prepareImagine", { modality });
...
async function generateVideoForPrompt(page, entry, attempt) {
  logStep("generateVideoForPrompt", { attempt });
  if (entry.filePath) {
...
async function generateImagesForPrompt(page, entry, promptIndex, state) {
  logStep("generateImagesForPrompt", { promptIndex, iterations: entry.iterations });
...
async function signupAndPrepare({ grokPage, browser }) {
  logStep("signupAndPrepare:start");
  let emailPage = null;
  ...
  logDebug("[turnstile] ready, submitting once.");
...
async function start() {
  ...
  while (currentIndex < prompts.length) {
    ...
    logStep("browser:connected");
    const stopWatcher = IS_CLOUD ? () => {} : startRateLimitWatcher(grokPage, state, 2000);
